...
se basan en aproximadamente una hora de datos recolectados el 3 de diciembre de 2025 (día de semana)
entre las 10:00 y las 11:00 (hora local de Vancouver).
...
fuerte sobreejecución como segmentos marcadamente más lentos que el plan. Creemos fuertemente que
las diferencias de velocidad se deben al horario y la congestión de la ciudad, la cual varía en función de la hora del día.
...
indica predominio de demoras por encima del horario programado, especialmente
debido a la hora de los datos y en calles troncales. Sin embargo, podemos ver 
...
servicio prolongadas. Debido a la hora de los datos, creemos que el headway analizado es representativo
del servicio un día normal de semana.
...
tasas de \emph{bunching} y \emph{gaps}. El análisis resultó en 2{,}724
observaciones (85 rutas, 2{,}221 paradas), con una diferencia promedio de
aprox.\ +5~min y mediana de +5.3~min; la tasa promedio de \emph{bunching} es
12\% y la de \emph{gaps} 54\%.
...
aparecen en la ruta 210 (centro), con demoras medias de headway $\sim$20~min
sobre lo programado (ej., Howe \& Pender, Pender \& Hamilton). En el extremo
opuesto, algunas paradas de las rutas 531 y 602 presentan headways reales
10--12~min más cortos que los programados, reflejando sobreoferta puntual.
...
\subsection{Retrasos por segmento con ocupación}

Para combinar desempeño operativo y nivel de ocupación se construyó la vista
materializada \texttt{qgis\_realtime\_load\_delay\_segments}. Parte de
\texttt{realtime\_load\_delay} (demora por tramo) y agrega por ruta, bucket de
ocupación y geometría de segmento (solo buses), calculando demora promedio,
desvío estándar y cantidad de observaciones, filtrando grupos con al menos tres
eventos. La consulta espera ocupación reciente en \texttt{rt\_vehicle\_positions}
que se pueda unir por \texttt{trip\_instance\_id}.

En este dataset no se registró ocupación asociable a los viajes observados, por
lo que la vista quedó vacía. De haberse contado con esos campos, el mapa
mostraría segmentos coloreados por bucket de ocupación y ordenados por demora
promedio, permitiendo ver si los tramos más demorados coinciden con vehículos
llenos.

\begin{figure}[H]
  \centering
  \includegraphicsifexists[width=0.8\textwidth]{%
    ../realtime_analysis/queries/results/load_delay_segments/load_delay_map.png}
  \caption{Mapa esperado de demora por segmento y bucket de ocupación
  (no generado por falta de datos de ocupación).}
  \label{fig:load-delay-map}
\end{figure}
...
representativa del servicio funcionando normalmente en un horario de trabajo. Los
mapas y distribuciones de demoras por parada evidencian una cola hacia valores
...
la mayoría de los buses tienden a circular de forma muy regular, con alrededor de 6 minutos de media, 
pero con más del 60\% de los buses circulando con menos de 3 minutos de intervalo, lo cual es un buen indicador de que el servicio es eficiente.
