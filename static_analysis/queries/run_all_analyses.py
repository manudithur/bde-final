...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run all static GTFS analyses",
...
existing results before generating new ones.
    ...
    QGIS_QUERIES_DIR = SCRIPT_DIR / "sql"
    QGIS_RUNNER = QGIS_QUERIES_DIR / "run_sql.py"
    ...
    scripts = [
        ...
        ("Population Density Analysis", VISUALIZATION_DIR / "population_density_analysis.py"),
        ("Stadium Proximity Analysis", VISUALIZATION_DIR / "stadium_proximity_analysis.py"),
        ("Stadium vs Population Analysis", VISUALIZATION_DIR / "stadium_population_analysis.py")
    ]
    ...
    # Step 1: Create materialized views from QGIS queries
    if not args.skip_sql:
        print("\n" + "="*60)
        print("STEP 1: Creating Materialized Views from QGIS Queries")
        print("="*60)
        
        if QGIS_RUNNER.exists():
            print(f"\nRunning QGIS query processor: {QGIS_RUNNER.name}")
            try:
                result = subprocess.run(
                    [sys.executable, str(QGIS_RUNNER)],
                    check=True,
                    capture_output=False,
                    cwd=str(QGIS_QUERIES_DIR)
                )
                ...
                print(f"\n✗ Error running QGIS query processor: {e}")
                print("  Continuing with visualizations (views may already exist)...")
        else:
            print(f"\n⚠ QGIS query runner not found: {QGIS_RUNNER}")
    ...
    print("="*60)
    print("\nNext steps:")
    print("  1. Materialized views (qgis_*) are ready for QGIS import")
